...
class ScenarioIot1 {
	...
	def static createBooleanVariable(String name, boolean value, ResourceSet rs) {
		val extension iotfactor = IotFactory::eINSTANCE
	...
	def static createIntVariable(String name, Integer value, ResourceSet rs) {
		val extension iotfactor = IotFactory::eINSTANCE
		val extension simpleexpressionsfactory = SimpleexpressionsFactory::eINSTANCE
	...
	def static void main(String[] args) {
		...
		val sese = SimpleexpressionsPackage::eINSTANCE
		val seiot = IotFactory::eINSTANCE
...
// 				action a1 comp { tooHot = temp >= tempThreshold, tooCold = temp < tempThreshold } in (e3) out (e5) service getTemp,
		val a1 = createOpaqueAction => [
			...
			incoming += e3
			outgoing += e5
			expressions += createExpressionBindExpression => [
				delegate = createIntegerComparisonExpression => [
					assignee = tooHot.delegate
					operand1 = temp.delegate
					operator = IntegerComparisonOperator.GREATER_EQUALS
					operand2 = tempThreshold.delegate
				]
			]
			expressions += createExpressionBindExpression => [
				delegate = createIntegerComparisonExpression => [
					assignee = tooCold.delegate
					operand1 = temp.delegate
					operator = IntegerComparisonOperator.SMALLER
					operand2 = tempThreshold.delegate
				]
			]
//			service getTemp
